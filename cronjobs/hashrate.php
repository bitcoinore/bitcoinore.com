...
$result = mysql_query($sql);
while ($resultrow = mysql_fetch_object($result)) {
	$teamhashrate = mysql_query("SELECT share_count, stale_share_count, shares_this_round, hashrate from webUsers WHERE teamId = '$resultrow->id'");
		while ($teamresultrow = mysql_fetch_object($teamhashrate)) {
		$hashrate = $teamresultrow->hashrate + $hashrate;
		$share_count = $teamresultrow->share_count + $share_count;
		$stale_share_count = $teamresultrow->stale_share_count + $stale_share_count;
		$shares_this_round = $teamresultrow->shares_this_round + $shares_this_round;
	mysql_query("UPDATE teams SET hashrate = $hashrate WHERE id = $resultrow->id");
	mysql_query("UPDATE teams SET share_count = $share_count WHERE id = $resultrow->id");
	mysql_query("UPDATE teams SET stale_share_count = $stale_share_count WHERE id = $resultrow->id");
	mysql_query("UPDATE teams SET shares_this_round = $shares_this_round WHERE id = $resultrow->id");
	}
}
